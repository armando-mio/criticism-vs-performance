"""Scarica i dati di performance da vaastav/Fantasy-Premier-League e li
filtra sulla sola squadra configurata (default: Liverpool).

Fonte dati: file statici su raw.githubusercontent.com, nessuna API key,
nessun rate limit da gestire (non e' un endpoint live).
...
def fetch_season(season_id: str, cfg: dict) -> dict[str, Path]:
    """Scarica e filtra i dati FPL per una singola stagione.

    Ritorna i path dei file scritti.
    """
    ...
    # 1) players_raw.csv -> anagrafica giocatori della squadra
    #    colonna "team" qui e' l'id numerico della squadra, non il nome.
    ...
    if team_row.empty:
        raise ValueError(f"Squadra '{team_name}' non trovata in teams.csv per {season_id}")
    ...
    # 2) merged_gw.csv -> statistiche gameweek per gameweek.
    #    qui la colonna "team" e' gia' il nome squadra in chiaro, si filtra diretto.
    ...
    print(
        f"[{season_id}] {len(team_players)} giocatori, "
        f"{len(team_gws)} righe gameweek -> {out_dir}"
    )
...
def main() -> None:
    ...
    parser.add_argument("--season", help="Solo questa stagione (es. 2024-25). Default: tutte.")
    ...
    for season_id in seasons:
        if season_id not in cfg["seasons"]:
            raise SystemExit(f"Stagione sconosciuta: {season_id}")
